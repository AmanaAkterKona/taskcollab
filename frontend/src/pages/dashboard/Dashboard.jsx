...
export default function Dashboard() {
  ...
  const { kpis, tasksByPriority = [], tasksByStatus = [], upcomingDeadlines = [], highPriorityTasks = [], projectSummary = [], memberWorkload = [] } = data || {};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title="Total Projects" value={kpis?.totalProjects} icon={FolderKanban} color="bg-primary-500" sub={`${kpis?.activeProjects} active`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-5">
          ...
          ) : <p className="text-gray-400 text-sm text-center py-10">No task data yet</p>}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-5 lg:col-span-1">
          ...
          <div className="space-y-4">
            {projectSummary.length === 0 && <p className="text-gray-400 text-sm">No projects yet</p>}
            {projectSummary.map((p) => (
              <div key={p._id}>
                ...
                <p className="text-xs text-gray-400 mt-0.5">{p.taskStats.pending} tasks pending</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card p-5 lg:col-span-1">
          ...
          <div className="space-y-3">
            {upcomingDeadlines.length === 0 && <p className="text-gray-400 text-sm">No upcoming deadlines</p>}
            {upcomingDeadlines.map((t) => (
              <div key={t._id} className="flex items-start gap-3">
                <div className="w-2 h-2 rounded-full bg-orange-400 mt-2 flex-shrink-0" />
                <div className="min-w-0">
                  <p className="text-sm font-medium truncate">{t.title}</p>
                  <p className="text-xs text-gray-400">{t.project?.name} · {format(new Date(t.dueDate), 'MMM d')}</p>
                </div>
        ...
        <div className="card p-5 lg:col-span-1">
          ...
          <div className="space-y-3">
            {(actData?.activities || []).length === 0 && <p className="text-gray-400 text-sm">No activity yet</p>}
            {(actData?.activities || []).slice(0, 8).map((a) => (
              <div key={a._id} className="flex items-start gap-3">
                <Avatar name={a.user?.name || '?'} size="sm" />
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">{a.description}</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {memberWorkload.map((m) => (
              <div key={m.user?._id} className="text-center">
                ...
                <p className="text-sm font-medium mt-2 truncate">{m.user?.name}</p>
                <div className="text-xs text-gray-500 space-y-0.5 mt-1">
                  <p>{m.total} tasks total</p>
                  <p className="text-green-600">{m.completed} done</p>
                  <p className="text-orange-500">{m.pending} pending</p>
                </div>
              </div>
            ))}
          </div>
