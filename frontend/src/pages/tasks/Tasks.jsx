...
import { CheckSquare, Search, Edit2, Trash2, MessageSquare, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">{tasks.length} tasks found</p>
      </div>
            ...
            <div key={t._id} className={`card p-4 border-l-4 ${priorityColor[t.priority]} hover:shadow-md transition-shadow`}>
              <div className="flex items-start gap-4">
                ...
                <div className="relative group mt-0.5">
                  <select
                    value={t.status}
                    onChange={(e) => statusMutation.mutate({ id: t._id, status: e.target.value })}
                    className="appearance-none cursor-pointer text-xs border-0 bg-transparent focus:outline-none"
                  >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className={`font-medium ${t.status === 'completed' ? 'line-through text-gray-400' : ''}`}>{t.title}</h3>
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => setViewTask(t)} className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400"><MessageSquare size={15} /></button>
                  {canManage && (
                    <>
      ...
      {viewTask && (
        <Modal isOpen={!!viewTask} onClose={() => setViewTask(null)} title={viewTask.title} size="lg">
          <div className="space-y-4">
            ...
            <h4 className="font-medium">Comments ({viewTask.comments?.length || 0})</h4>
            <div className="space-y-3 max-h-48 overflow-y-auto">
              {(viewTask.comments || []).map((c) => (
                <div key={c._id} className="flex gap-3">
                  <Avatar name={c.user?.name || '?'} size="sm" />
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2 flex-1">
                    <p className="text-xs font-medium text-gray-500">{c.user?.name}</p>
                    <p className="text-sm mt-0.5">{c.text}</p>
                  </div>
                </div>
              ))}
              {!viewTask.comments?.length && <p className="text-sm text-gray-400">No comments yet</p>}
            </div>
